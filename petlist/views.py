...
#for showing signup/login button for rescuer
def mechanicsclick_view(request):
    if request.user.is_authenticated:
        return HttpResponseRedirect('afterlogin')
    return render(request,'vehicle/rescuerclick.html')
...
def mechanic_signup_view(request):
    ...
    if request.method=='POST':
        ...
        return HttpResponseRedirect('rescuerlogin')
    return render(request,'vehicle/rescuersignup.html',context=mydict)


#for checking user customer, rescuer or admin
...
def admin_mechanic_view(request):
    return render(request,'vehicle/admin_rescuer.html')


@login_required(login_url='adminlogin')
def admin_approve_mechanic_view(request):
    mechanics=models.Mechanic.objects.all().filter(status=False)
    return render(request,'vehicle/admin_approve_rescuer.html',{'mechanics':mechanics})
...
def approve_mechanic_view(request,pk):
    ...
    if request.method=='POST':
        ...
        return HttpResponseRedirect('/admin-approve-rescuer')
...
def delete_mechanic_view(request,pk):
    ...
    user.delete()
    mechanic.delete()
    return redirect('admin-approve-rescuer')
...
def admin_add_mechanic_view(request):
    ...
    if request.method=='POST':
        ...
        if userForm.is_valid() and mechanicForm.is_valid() and mechanicSalary.is_valid():
            ...
            my_mechanic_group = Group.objects.get_or_create(name='MECHANIC')
            my_mechanic_group[0].user_set.add(user)
            return HttpResponseRedirect('admin-view-rescuer')
        else:
            print('problem in form')
    return render(request,'vehicle/admin_add_rescuer.html',context=mydict)


@login_required(login_url='adminlogin')
def admin_view_mechanic_view(request):
    mechanics=models.Mechanic.objects.all()
    return render(request,'vehicle/admin_view_rescuer.html',{'mechanics':mechanics})
...
def delete_mechanic_view(request,pk):
    ...
    user.delete()
    mechanic.delete()
    return redirect('admin-view-rescuer')
...
def admin_view_mechanic_salary_view(request):
    mechanics=models.Mechanic.objects.all()
    return render(request,'vehicle/admin_view_rescuer_salary.html',{'mechanics':mechanics})
...
def update_salary_view(request,pk):
    ...
    if request.method=='POST':
        ...
        if mechanicSalary.is_valid():
            ...
            print("form is invalid")
        return HttpResponseRedirect('/admin-view-rescuer-salary')
    return render(request,'vehicle/admin_approve_mechanic_details.html',{'mechanicSalary':mechanicSalary})
...
def admin_mechanic_attendance_view(request):
    return render(request,'vehicle/admin_rescuer_attendance.html')
...
def admin_report_view(request):
    reports=models.Request.objects.all().filter(Q(status="Work Done") | Q(status="Released"))
    dict={
        'reports':reports,
    }
...
# CUSTOMER/CLIENT RELATED views start
...
def customer_dashboard_view(request):
    ...
    work_in_progress=models.Request.objects.all().filter(customer_id=customer.id,status='Rescuing/Turnin').count()
    work_completed=models.Request.objects.all().filter(customer_id=customer.id).filter(Q(status="Work Done") | Q(status="Released")).count()
    ...
    bill=models.Request.objects.all().filter(customer_id=customer.id).filter(Q(status="Work Done") | Q(status="Released")).aggregate(Sum('cost'))
    print(bill)
    dict={
...
# Rescuer RELATED views start



@login_required(login_url='rescuerlogin')
...
def mechanic_dashboard_view(request):
    ...
    work_in_progress=models.Request.objects.all().filter(mechanic_id=mechanic.id,status='Rescuing/Turnin').count()
    work_completed=models.Request.objects.all().filter(mechanic_id=mechanic.id,status='Work Done').count()
    ...
    return render(request,'vehicle/rescuer_dashboard.html',context=dict)
...
def mechanic_work_assigned_view(request):
    ...
    return render(request,'vehicle/rescuer_work_assigned.html',{'works':works,'mechanic':mechanic})
...
def mechanic_update_status_view(request,pk):
    ...
    if request.method=='POST':
        ...
        if updateStatus.is_valid():
            ...
            print("form is invalid")
        return HttpResponseRedirect('/rescuer-work-assigned')
...
def mechanic_feedback_view(request):
    ...
    return render(request,'vehicle/rescuer_feedback.html',{'feedback':feedback,'mechanic':mechanic})

@login_required(login_url='rescuerlogin')
@user_passes_test(is_mechanic)
def mechanic_salary_view(request):
    mechanic=models.Mechanic.objects.get(user_id=request.user.id)
    workdone=models.Request.objects.all().filter(mechanic_id=mechanic.id).filter(Q(status="Work Done") | Q(status="Released"))
    return render(request,'vehicle/rescuer_salary.html',{'workdone':workdone,'mechanic':mechanic})

@login_required(login_url='rescuerlogin')
@user_passes_test(is_mechanic)
def mechanic_profile_view(request):
    mechanic=models.Mechanic.objects.get(user_id=request.user.id)
    return render(request,'vehicle/rescuer_profile.html',{'mechanic':mechanic})
...
def edit_mechanic_profile_view(request):
    ...
    if request.method=='POST':
        ...
        if userForm.is_valid() and mechanicForm.is_valid():
            ...
            user.save()
            mechanicForm.save()
            return redirect('rescuer-profile')
    return render(request,'vehicle/edit_rescuer_profile.html',context=mydict)
